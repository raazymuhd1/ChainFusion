import React from 'react'
import { countersData } from '@/constants'

const Counters = () => {
  return (
    <section className={`grid mt-[80px] grid-cols-[repeat(auto-fit,minmax(100px,1fr))] w-[40%] justify-items-center mx-auto gap-[20px]`}>
        { countersData.map((count, idx) => (
            <aside
                key={idx}
                className='flex flex-col items-center gap-[5px] cursor-pointer hover:scale-[1.1] transition-[scale_500ms] '>
               <h2 className='font-bold text-[clamp(1.5rem,1vw,2rem)] text-[var(--warning)]'> { count.value } </h2>
               <h4 className='text-[#9090a8]'> { count.title } </h4>
            </aside>
        )) }
    </section>
  )
}

export default Counters